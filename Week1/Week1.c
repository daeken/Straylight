...
BOOL gridText[GRID_WIDTH*GRID_HEIGHT];

#define SQ(v) ((v) * (v))
#define WAVESPEED 0.3
// WAVESPEED was 0.3
#define WAVELIFE 0.1
// WAVELIFE was 0.1
#define GRIDSIZE 0.5
...
void setText(char *text) {
	...
	memset(gridText, 0, sizeof(BOOL) * GRID_WIDTH * GRID_HEIGHT);
	
	for(y = 0; y < 4; ++y) {
		for(x = 0; x < len; ++x) {
			int pos = (y + GRID_HH - 2) * GRID_WIDTH + startx + (x * 5);
			
			gridText[pos  ] = chars[text[x] - 'A'][y * 4];
			gridText[pos+1] = chars[text[x] - 'A'][y * 4 + 1];
			gridText[pos+2] = chars[text[x] - 'A'][y * 4 + 2];
			gridText[pos+3] = chars[text[x] - 'A'][y * 4 + 3];
		}
	}
}
...
void updateGrid() {
	...
	memset(grid, 0, sizeof(float) * GRID_HEIGHT * GRID_WIDTH);
...
void checktime() {
	...
	if(time > 30 && time < 45)
		...
		setText("WAVERIDE");
	else if(time > 60 && time < 105)
		setText("");
	else if(time > 105 && time < 120)
		setText("NIGHTBEAT");
	else if(time > 120 && time < 135)
		setText("ASD");
	else if(time > 135 && time < 150)
		setText("SVATG");
...
void render() {
	int i, x, y, off = 0;
	float prev = 0.0;
	GLfloat position[] = {0.0f, 25.0f, -ZOOM, 1.0f};
